...
export function UserMenu({ onOpenSecuritySettings }: UserMenuProps) {
  ...
  const formatExpiry = (expiry: string | null) => {
    if (!expiry) return 'Sem expiração';
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="sm" className="gap-2" title="Menu da sessão">
          <Lock className="h-4 w-4 text-emerald-500" />
          <span className="text-xs text-muted-foreground hidden sm:inline">Protegido</span>
